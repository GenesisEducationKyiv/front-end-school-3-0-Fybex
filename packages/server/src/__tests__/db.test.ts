  ...
  describe('getTracks', () => {
    ...
    beforeAll(async () => {
      ...
      for (let i = 0; i < expectedTrackCount; i++) {
        await createTrack({
          title: `Pagination Test Track ${i.toString()}`,
          artist: 'Pagination Artist',
          genres: ['Electronic'],
          slug: `pagination-test-track-${i.toString()}`,
        });
      }
    });
    ...
    it('should get all tracks with pagination', async () => {
      ...
      // We should have at least some tracks (the ones we created in this describe block)
      // Note: Other tests might clean up the database, so we check for at least 1 track
